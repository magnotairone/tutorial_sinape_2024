...
Entretanto, é possível integrá-los com Python através do pacote `reticulate`, como veremos no tutorial.

Todo trecho de código apresentado aqui terá uma marcação no título indicando se é [R (cor azul)]{style="background-color: rgba(37, 105, 187, 0.4);"} ou [Python (cor amarela)]{style="background-color: rgba(241, 196, 15, 0.4);"}.
...
No entanto, a criação de ambientes no Python oferece vantagens significativas, como isolamento, reprodutibilidade e melhor gestão de dependências.
...
Você pode então trabalhar com o objeto como qualquer outro objeto R.
...
Vamos utilziar o modelo "text-embedding-ada-002" da OpenAI para gerar embeddings a partir do conteúdo do PDF:
...
Aqui, criamos um índice específico para nossos dados chamado `"tutorial-sinape"`:
...
Agora, podemos usar diretamente o objeto armezanado na variável `vectorstore_from_docs` ou então, carregar o vetor que está armazenado na Pinecone.
...
MaritacaAI é uma plataforma de inteligência artificial desenvolvida no Brasil, focada em fornecer soluções personalizadas de processamento de linguagem natural e aprendizado de máquina.
...
Você ganhará R\$20 de créditos para fazer seus testes.
...
Aqui escolhemos trabalhar com o modelo `"sabia-2-small"`, o que apresenta o melhor custo-benefício, custando R\$1 por milhão de tokens de entrada e R\$3 por milhão de token de saída.
